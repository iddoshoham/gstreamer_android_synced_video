...
typedef struct _Player {
    ...
    GstNetTimeProvider *net_time_provider; // Server
    GstClock *net_client_clock;            // Client
} Player;
...
on_video_dimensions_changed(GstPlayer *unused, gint width, gint height,
    ...
    (*env)->CallVoidMethod(env, player->java_player,
                           on_video_dimensions_changed_method_id, width,
                           height);
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionDescribe(env);
        (*env)->ExceptionClear(env);
...
native_free(JNIEnv *env, jobject thiz) {
    ...
    if (player->net_time_provider) {
        g_object_unref(player->net_time_provider);
        player->net_time_provider = NULL;
    }

    if (player->net_client_clock) {
        gst_object_unref(player->net_client_clock);
        player->net_client_clock = NULL;
    }
...
/* Set up NetTimeProvider server */
static GstClockTime
native_enable_time_provider(JNIEnv *env, jobject thiz, jint port) {
    Player *player = GET_CUSTOM_DATA (env, thiz, native_player_field_id);
...
//    GstClock *clock = gst_pipeline_get_clock (GST_PIPELINE (player->player));
    GstClock *clock = gst_system_clock_obtain();
    player->net_time_provider = gst_net_time_provider_new(clock, NULL, port);

    GST_DEBUG ("Enabled NetTimeProvider on port %d", port);

    GstClockTime *base = NULL;
    base = g_new(GstClockTime,
                 1); //g_malloc(sizeof(GstClockTime));//gst_clock_get_time(shared.clock);
    GstClockTime currTime = gst_clock_get_time(clock);

    *base = currTime;    //gst_clock_get_time(shared.clock);
    return *base;

}

/* Set up NetClientClock client */
static void
native_set_net_client_clock(JNIEnv *env, jobject thiz, jstring ip, jint port,
                            jlong base) {
    GST_WARNING("Set NetClientClock1 from server %s:%d", ip, port);
    Player *player = GET_CUSTOM_DATA (env, thiz, native_player_field_id);
    const gchar *ip_str;
    ...
    gst_element_set_base_time(pipeline,
                              base
                              + (GST_SECOND * 5));
    gst_pipeline_use_clock(GST_PIPELINE (pipeline), player->net_client_clock);
//    gst_object_unref (player->net_client_clock);
...
native_class_init(JNIEnv *env, jclass klass) {
    ...
    on_error_method_id =
            (*env)->GetMethodID(env, klass, "onError",
                                "(ILjava/lang/String;)V");
    on_video_dimensions_changed_method_id =
            (*env)->GetMethodID(env, klass, "onVideoDimensionsChanged",
                                "(II)V");
